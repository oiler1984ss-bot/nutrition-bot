...
async def generate_menu(data):
    ...
    prompt = f"""Ты профессиональный нутрициолог. Составь ДЕТАЛЬНОЕ меню на 7 дней.

ПАРАМЕТРЫ:
- {data['gender']}, {data['age']} лет, {weight} кг, {data['height']} см
- Цель: {data['target']}, {data.get('activity', 'unknown')}
- Норма: {data['calories']} ккал | Б:{protein}г Ж:{fat}г У:{carbs}г К:{fiber}г
- Вода: {water_min}-{water_max} л

СТРУКТУРА каждого дня:
=== ПОНЕДЕЛЬНИК ===
ЗАВТРАК (08:00): блюдо, ингредиенты, КБЖУ+К
ПЕРЕКУС (11:00): продукты, КБЖУ+К
ОБЕД (14:00): блюдо, ингредиенты, КБЖУ+К
ПЕРЕКУС (17:00): продукты, КБЖУ+К
УЖИН (20:00): блюдо, ингредиенты, КБЖУ+К
ПОЧЕМУ: 2-3 предложения о пользе
ИТОГ: ккал|Б|Ж|У|К
💧 Вода: {water_min}-{water_max} л

[Повторите для ВТОРНИК, СРЕДА, ЧЕТВЕРГ, ПЯТНИЦА, СУББОТА, ВОСКРЕСЕНЬЕ]

ВАЖНО: Все 7 дней! Реальные продукты. Точные граммовки. Без #*_`"""
    ...
    try:
        ...
        logging.info(f"Generated: {list(final_menu.keys())}")
...
async def ask_nutritionist(user_data, question):
    prompt = f"""ИИ-нутрициолог. Данные: {user_data['gender']}, {user_data['age']} лет, {user_data['weight']} кг.
Цель: {user_data['target']}. Норма: {user_data['daily_calories']} ккал.
Вопрос: "{question}"
Отвечай дружелюбно, подробно (5-10 предложений), без #*_`."""
